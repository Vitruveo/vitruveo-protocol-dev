...
func (api *API) IntermediateRoots(ctx context.Context, hash common.Hash, config *TraceConfig) ([]common.Hash, error) {
	...
	for i, tx := range block.Transactions() {
		...
		if _, err := core.ApplyMessage(5, vmenv, msg, new(core.GasPool).AddGas(msg.GasLimit)); err != nil {
...
txloop:
	for i, tx := range txs {
		...
		if _, err := core.ApplyMessage(6, vmenv, msg, new(core.GasPool).AddGas(msg.GasLimit)); err != nil {
			failed = err
			break txloop
		}
		// Finalize the state so any modifications are written to the trie
...
func (api *API) standardTraceBlockToFile(ctx context.Context, block *types.Block, config *StdTraceConfig) ([]string, error) {
	...
	for i, tx := range block.Transactions() {
		...
		statedb.SetTxContext(tx.Hash(), i)
		_, err = core.ApplyMessage(7, vmenv, msg, new(core.GasPool).AddGas(msg.GasLimit))
		if writer != nil {
			writer.Flush()
		}
		if dump != nil {
...
func (api *API) traceTx(ctx context.Context, message *core.Message, txctx *Context, vmctx vm.BlockContext, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	if _, err = core.ApplyMessage(8, vmenv, message, new(core.GasPool).AddGas(message.GasLimit)); err != nil {
