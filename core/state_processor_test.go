...
func GenerateBadBlock(parent *types.Block, engine consensus.Engine, txs types.Transactions, config *params.ChainConfig) *types.Block {
	...
	if config.IsShanghai(header.Number, header.Time) {
		return types.NewBlockWithWithdrawals(parent.Header(), header, txs, nil, receipts, []*types.Withdrawal{}, trie.NewStackTrie(nil))
	}
	return types.NewBlock(parent.Header(), header, txs, nil, receipts, trie.NewStackTrie(nil))
}
