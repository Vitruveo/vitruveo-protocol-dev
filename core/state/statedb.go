...
// GetRebasedBalance retrieves the balance from the given address or 0 if object not found
func (s *StateDB) GetRebasedBalance(addr common.Address, rbx uint64) *big.Int {
	stateObject := s.getStateObject(addr)
	if stateObject != nil {
		return rebase.GetRebasedAmount(stateObject.Balance(), rbx)
	}
	return common.Big0
}
