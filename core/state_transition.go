...
func ApplyMessage(caller int, evm *vm.EVM, msg *Message, gp *GasPool) (*ExecutionResult, error) {
	return NewStateTransition(caller, evm, msg, gp).TransitionDb()
}
...
func NewStateTransition(caller int, evm *vm.EVM, msg *Message, gp *GasPool) *StateTransition {
...
func (st *StateTransition) buyGas() error {
	...
	if have, want := st.state.GetRebasedBalance(st.msg.From, st.evm.Context.Rbx), balanceCheck; have.Cmp(want) < 0 {
...
func (st *StateTransition) TransitionDb() (*ExecutionResult, error) {
	...
	// Check clause 6
	if msg.Value.Sign() > 0 && !st.evm.Context.CanTransfer(st.state, st.evm.Context.Rbx, msg.From, msg.Value) {
