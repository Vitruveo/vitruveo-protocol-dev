...
// rules without duplicating code and running the risk of missed updates.
func ValidateTransactionWithState(tx *types.Transaction, signer types.Signer, opts *ValidationOptionsWithState, rbx uint64) error {
	...
	var (
		balance = opts.State.GetRebasedBalance(from, rbx)
		cost    = tx.Cost()
	)
