...
func (p *BlobPool) recheck(addr common.Address, inclusions map[common.Hash]uint64) {
	...
	var (
		balance = uint256.MustFromBig(p.state.GetRebasedBalance(addr, p.chain.CurrentBlock().Rbx))
		spent   = p.spent[addr]
	)
...
func (p *BlobPool) validateTx(tx *types.Transaction) error {
	...
	if err := txpool.ValidateTransactionWithState(tx, p.signer, stateOpts, p.chain.CurrentBlock().Rbx); err != nil {
		return err
	}
