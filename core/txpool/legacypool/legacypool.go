...
func (pool *LegacyPool) validateTx(tx *types.Transaction, local bool) error {
	...
	if err := txpool.ValidateTransactionWithState(tx, pool.signer, opts, pool.chain.CurrentBlock().Rbx); err != nil {
		return err
	}
	return nil
}
...
func (pool *LegacyPool) promoteExecutables(accounts []common.Address) []*types.Transaction {
	...
	for _, addr := range accounts {
		...
		drops, _ := list.Filter(pool.currentState.GetRebasedBalance(addr, pool.chain.CurrentBlock().Rbx), gasLimit)
...
func (pool *LegacyPool) demoteUnexecutables() {
	...
	for addr, list := range pool.pending {
		...
		drops, invalids := list.Filter(pool.currentState.GetRebasedBalance(addr, pool.chain.CurrentBlock().Rbx), gasLimit)
		for _, tx := range drops {
			hash := tx.Hash()
