...
func (b *BlockGen) addTx(bc *BlockChain, vmConfig vm.Config, tx *types.Transaction) {
	...
	currentRbx := uint64(0)
	if b.header.Rbx == 0 {
		currentRbx = bc.GetBlockByNumber(b.header.Number.Sub(b.header.Number, big.NewInt(1)).Uint64()).Header().Rbx
	}
	receipt, err := ApplyTransaction(b.config, bc, &b.header.Coinbase, b.gasPool, b.statedb, b.header, tx, &b.header.GasUsed, vmConfig, currentRbx)
	if err != nil {
		panic(err)
	}
...
func (b *BlockGen) GetBalance(addr common.Address) *big.Int {
	return b.statedb.GetRebasedBalance(addr, b.header.Rbx)
}
